...
from octo_oxe_data_utils import obs_transforms, traj_transforms
from octo_oxe_data_utils.utils import goal_relabeling, task_augmentation
...
def make_dataset_from_rlds(
    ...
    *,
    ...
    def restructure(traj):
        ...
        print("obj_obs", old_obs.keys())
        print("state_obs.keys, ", state_obs_keys)
        new_obs = {}
    ...
    # )
    ## TODO merge datasets of different split
    splits = builder.info.splits
    first_split = list(splits.keys())[0]
    # merged_dataset = builder.as_dataset(split=first_split)
    dataset = dl.DLataset.from_rlds(
        builder, split=first_split, shuffle=False, num_parallel_reads=num_parallel_reads
    )
    for split_name in list(splits.keys())[1:]:
        dataset_tmp =dl.DLataset.from_rlds(
            builder, split=split_name, shuffle=False, num_parallel_reads=num_parallel_reads
        )
    ...
    return dataset, dataset_statistics, builder.info
...
def make_single_dataset(
    ...
    *,
    ...
    """
    dataset, dataset_statistics, info = make_dataset_from_rlds(
        **dataset_kwargs,
        train=train,
    )
